...
{
    const std::size_t _compatibilityLevel = MODULE_COMPATIBILITY_LEVEL;

public:
    RegisterableModule() = default;
     ...
     *
     * A RegisterableModule must NOT invoke any calls to other modules in its constructor,
     * since at the point of construction the other modules will not have been
     * initialised. Once all of the dependencies are processed by the ModuleRegistry, each
     * module will have its initialiseModule() method called at the appropriate point.
     *
     * The ModuleRegistry guarantees that any modules named in the set of dependencies
     * returned by getDependencies() will be initialised and ready for use at the time
     * this method is invoked. Attempting to access a module which was NOT listed as a
